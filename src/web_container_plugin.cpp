...
static Eina_Bool run_javascript_alert_cb(Evas_Object* o, const char* alert_text, void* user_data)
{
    LOGD("ALERT : %s", alert_text);
    return EINA_FALSE;
}
...
bool web_container_create(Evas_Object *window, HelperInfoEx info, web_container_loaded_cb callback)
{
    ...
    if (wrt) {
        ...
        if (_pCurrentWebView) {
            //ewk_view_javascript_alert_callback_set(_pCurrentWebView, run_javascript_alert_cb, NULL);
        }
